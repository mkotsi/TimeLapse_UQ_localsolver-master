...
class OptimizationBase(object):
    ...
    def __init__(self, objective, geom_fac = 0.6, geom_fac_up = 0.7):
        ...
        * InversionMethodsType must have member functions that implement the
        ...
        self.baseline_model_set = False
        
    def set_baseline_model_ddfwi(self, m0):
        self.m0 = m0
        self.baseline_model_set = True
...
#   #### Actual optimization stuff below...
    ...
    def __call__(self,
        ...
        if np.iterable(iteration_parameters):
            for ip in iteration_parameters:
                ...
                # Call the inner loop
                self.inner_loop(shots, beta, model_reg_term_scale, steps, objective_arguments=arguments, **kwargs)
        else:
            if type(iteration_parameters) is int:
                # Call the inner loop
                steps=iteration_parameters
                self.inner_loop(shots, beta, model_reg_term_scale, steps, **kwargs)
            else:
    ...
    def regularized_objective(self, shots, m1, beta, model_reg_term_scale, objective_arguments={}):
        objective_from_data = self.objective_function.evaluate(shots, m1, **objective_arguments)
        
        model_diff = self.m0 - m1 #perturbation
        take_norm_squared_of_this = model_diff*(1/beta) #ndarray
        take_norm_squared_of_this = m1.perturbation(data=take_norm_squared_of_this) #make perturbation of it again
        ...
        objective_combined = objective_from_data - objective_value_contrib_from_beta
        return objective_combined

    def regularized_gradient(self, shots, m1, beta, model_reg_term_scale, objective_arguments={}, aux_info={}):
        ...
        model_diff = self.m0 - m1 #subtraction gives a perturbation class back, so it is in linear form

        gradient_m1_contrib_from_beta = model_diff*(-model_reg_term_scale*(1/beta)**2)  #no 0.5* here. NDARRAY
        gradient_m1_contrib_from_beta = m1.perturbation(data=gradient_m1_contrib_from_beta)
        ...
        gradient_combined = gradient + gradient_m1_contrib_from_beta
        objective_value_combined = objective_value + objective_value_contrib_from_beta

        if self.objective_function.parallel_wrap_shot.rank == 0:
            self._print("STORING HISTORY SHOULD PROBABLY NOT HAPPEN IN REGULARIZED_GRADIENT. SHOULD PUT BACK OUTSIDE IN INNER_LOOP. \n")
            
            #FOR DEBUGGING PURPOSES
            self._print('objective value from data: {0}'.format(objective_value))
            self._print('objective value from beta: {0}'.format(objective_value_contrib_from_beta))
        
            self._print('gradient norm from data: {0}'.format(gradient.norm()))
            self._print('gradient norm from beta: {0}'.format(gradient_m1_contrib_from_beta.norm()))
         
        return gradient_combined, objective_value_combined
        

    def inner_loop(self, shots, beta, model_reg_term_scale, steps, objective_arguments={}, **kwargs):
        ...
        if not self.baseline_model_set:
            raise Exception("Should set m0 before starting!")
        ...
        for step in xrange(steps):
            ...
            # Compute the gradient
            gradient, objective_value = self.regularized_gradient(shots, self.base_model, beta, model_reg_term_scale, objective_arguments = objective_arguments, aux_info = aux_info)
            ...
            step = self._select_step(shots, beta, model_reg_term_scale, objective_value, gradient, i, objective_arguments, **kwargs)
    ...
    def select_alpha(self, shots, beta, model_reg_term_scale, gradient, direction, objective_arguments, **kwargs):
        """Resets the state of the optimization algorithm.
        ...
        if self.ls_method == 'constant':
            ...
            return self._linear_line_search(shots, beta, model_reg_term_scale, gradient, direction, objective_arguments, **kwargs)

        elif self.ls_method == 'backtrack':
            return self._backtrack_line_search(shots, beta, model_reg_term_scale, gradient, direction, objective_arguments, **kwargs)
    ...
    def _linear_line_search(self, shots, beta, model_reg_term_scale, gradient, direction, objective_arguments, **kwargs):
...
#       return numerator / denominator
    ...
    def _backtrack_line_search(self, shots, beta, model_reg_term_scale, gradient, direction, objective_arguments,
        ...
        if current_objective_value is None:
            fk = self.regularized_objective(shots, self.base_model, beta, model_reg_term_scale, objective_arguments)
        else:
        ...
        while not stop:
            ...
            fkp1 = self.regularized_objective(shots, model, beta, model_reg_term_scale, objective_arguments)
