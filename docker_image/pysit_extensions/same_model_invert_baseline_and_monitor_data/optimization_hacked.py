...
class OptimizationBase(object):
    ...
    def __init__(self, objective_0, objective_1, geom_fac = 0.6, geom_fac_up = 0.7, store_shot_contributions = False):
        ...
        * InversionMethodsType must have member functions that implement the
        ...
        self.store_shot_contributions = store_shot_contributions
        
    def reset(self,
              append_mode,
              value_frequency=0,
              gradient_frequency=0,
              gradient_0_frequency=0,
              gradient_1_frequency=0,
              gradient_length_frequency=0,
              step_frequency=0,
              step_length_frequency=0,
              residual_length_0_frequency=0,
              residual_length_1_frequency=0,
              objective_0_frequency=0,
              objective_1_frequency=0,
              run_time_frequency=0,
              alpha_frequency=1,
              pseudo_hess_diag_0_frequency=0,
              pseudo_hess_diag_1_frequency=0,   
              shot_contributions_0_frequency=0,
              shot_contributions_1_frequency=0,           
              *args, **kwargs):
        ...
        if not append_mode or not hasattr(self, 'iteration'):
            ...
            self.init_history("gradient_0",             gradient_0_frequency)
            self.init_history("gradient_1",             gradient_1_frequency)
            ...
            self.init_history("residual_length_0",      residual_length_0_frequency)    # Requires self.residual_norm to be implemented by residual class if L2 is not appropriate
            self.init_history("residual_length_1",      residual_length_1_frequency)    # Requires self.residual_norm to be implemented by residual class if L2 is not appropriate
            self.init_history("objective_0",            objective_0_frequency)
            self.init_history("objective_1",            objective_1_frequency)
            self.init_history("run_time",               run_time_frequency)
            self.init_history("pseudo_hess_diag_0",     pseudo_hess_diag_0_frequency)
            self.init_history("pseudo_hess_diag_1",     pseudo_hess_diag_1_frequency)
            if self.store_shot_contributions:
                self.init_history("shot_contributions_0",   shot_contributions_0_frequency)
                self.init_history("shot_contributions_1",   shot_contributions_1_frequency)
...
#   #### Actual optimization stuff below...
    ...
    def __call__(self,
        ...
        if np.iterable(iteration_parameters):
            for ip in iteration_parameters:
                ...
                # Call the inner loop
                self.inner_loop(shots_0, shots_1, steps, objective_arguments=arguments, **kwargs)
        else:
            if type(iteration_parameters) is int:
                # Call the inner loop
                steps=iteration_parameters
                self.inner_loop(shots_0, shots_1, steps, **kwargs)
            else:
    ...
    def inner_loop(self, shots_0, shots_1, steps, objective_arguments={}, **kwargs):
        ...
        for step in xrange(steps):
            ...
            aux_info_0 = {'objective_value'     : (True, None),
                          'residual_norm'       : (True, None),
                          'pseudo_hess_diag'    : (True, None)}
            
            aux_info_1 = {'objective_value'     : (True, None),
                          'residual_norm'       : (True, None),
                          'pseudo_hess_diag'    : (True, None)}

            if self.store_shot_contributions:
                aux_info_0['shot_contributions'] = (True, None)
                aux_info_1['shot_contributions'] = (True, None)

            # COMPUTE THE GRADIENT
            gradient_0 = self.objective_function_0.compute_gradient(shots_0, self.base_model, aux_info=aux_info_0, **objective_arguments)
            gradient_1 = self.objective_function_1.compute_gradient(shots_1, self.base_model, aux_info=aux_info_1, **objective_arguments)
            gradient = gradient_0 + gradient_1 # + model reg term optionally
            
            #OBJECTIVE VALUE
            objective_value_0 = aux_info_0['objective_value'][1]
            objective_value_1 = aux_info_1['objective_value'][1]
            objective_value   = objective_value_0 + objective_value_1  # + model reg term optionally
            
            #SHOT CONTRIBUTIONS
            if self.store_shot_contributions:
                self.store_history('shot_contributions_0', i, aux_info_0['shot_contributions'][1])
                self.store_history('shot_contributions_1', i, aux_info_1['shot_contributions'][1])
            
            
            # Process and store meta data about the gradient
            self.store_history('gradient_0', i, gradient_0)
            self.store_history('gradient_1', i, gradient_1)
            ...
            if objective_value_0 is not None:
                self.store_history('objective_0', i, objective_value_0)
                self._print('  objective_0 {0}'.format(objective_value_0))
                            
            if objective_value_1 is not None:
                self.store_history('objective_1', i, objective_value_1)
                self._print('  objective_1 {0}'.format(objective_value_1))

            if aux_info_0['residual_norm'][1] is not None:
                self.store_history('residual_length_0', i, aux_info_0['residual_norm'][1])
                self._print('  residual_0 {0}'.format(aux_info_0['residual_norm'][1]))

            if aux_info_1['residual_norm'][1] is not None:
                self.store_history('residual_length_1', i, aux_info_1['residual_norm'][1])
                self._print('  residual_1 {0}'.format(aux_info_1['residual_norm'][1]))

            if aux_info_0['pseudo_hess_diag'][1] is not None:
                self.store_history('pseudo_hess_diag_0', i, aux_info_0['pseudo_hess_diag'][1])
                self._print('  norm pseudo_hess_diag_0 {0}'.format(np.linalg.norm(aux_info_0['pseudo_hess_diag'][1]))) #Not really useful. Printing right now to see if nothing strange happens.

            if aux_info_1['pseudo_hess_diag'][1] is not None:
                self.store_history('pseudo_hess_diag_1', i, aux_info_1['pseudo_hess_diag'][1])
                self._print('  norm pseudo_hess_diag_1 {0}'.format(np.linalg.norm(aux_info_1['pseudo_hess_diag'][1]))) #Not really useful. Printing right now to see if nothing strange happens.            

            # Compute step modifier
            step = self._select_step(shots_0, shots_1, objective_value, gradient, i, objective_arguments, **kwargs)
    ...
    def select_alpha(self, shots_0, shots_1, gradient, direction, objective_arguments, **kwargs):
        """Resets the state of the optimization algorithm.
        ...
        if self.ls_method == 'constant':
            ...
            return self._linear_line_search(shots_0, shots_1, gradient, direction, objective_arguments, **kwargs)

        elif self.ls_method == 'backtrack':
            return self._backtrack_line_search(shots_0, shots_1, gradient, direction, objective_arguments, **kwargs)
...
#       return numerator / denominator
    ...
    def _backtrack_line_search(self, shots_0, shots_1,gradient, direction, objective_arguments,
        ...
        if current_objective_value is None:
            obj_0 = self.objective_function_0.evaluate(shots_0, self.base_model, **objective_arguments)
            obj_1 = self.objective_function_1.evaluate(shots_1, self.base_model, **objective_arguments)
            fk = obj_0 + obj_1
        else:
        ...
        while not stop:
            ...
            obj_0 = self.objective_function_0.evaluate(shots_0, model, **objective_arguments)
            obj_1 = self.objective_function_1.evaluate(shots_1, model, **objective_arguments)
