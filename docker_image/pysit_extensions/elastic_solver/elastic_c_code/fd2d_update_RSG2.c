...
#pragma omp for
  for(k=ik;k<kk-1;k++) {
    for(m=1;m<mm-1;m++) {
      ...
      d1  = vx[GI(m+1,k+1)]-vx[GIJ];
      d2  = vx[GI(m+1,k)]-vx[GI(m,k+1)];
      exx = 0.5*(d1+d2);
      exz = 0.5*(d1-d2);
      d1  = vz[GI(m+1,k+1)]-vz[GIJ];
      d2  = vz[GI(m+1,k)]-vz[GI(m,k+1)];
      ezx = 0.5*(d1+d2);
      ezz = 0.5*(d1-d2);
...
#pragma omp for
  for(k=ik;k<kk-1;k++) {
    for(m=1;m<mm-1;m++) {
       ...
       d1 = txx[GIJ]-txx[GI(m-1,k-1)];
       d2 = txx[GI(m,k-1)]-txx[GI(m-1,k)];
       xtxx = 0.5*(d1+d2);
       d1 = tzz[GIJ]-tzz[GI(m-1,k-1)];
       d2 = tzz[GI(m,k-1)]-tzz[GI(m-1,k)];
       ztzz = 0.5*(d1-d2);
       d1 = txz[GIJ]-txz[GI(m-1,k-1)];
       d2 = txz[GI(m,k-1)]-txz[GI(m-1,k)];
       xtxz = 0.5*(d1+d2);
       ztxz = 0.5*(d1-d2);
