...
class LBFGS(OptimizationBase):
    ...
    def __init__(self, objective_0, objective_1 = None, memory_length=None, reset_on_new_inner_loop_call=True, geom_fac = 0.6, geom_fac_up = 0.7, scale_step = False, *args, **kwargs):
        ...
        OptimizationBase.__init__(self, objective_0, objective_1, geom_fac = geom_fac, geom_fac_up = geom_fac_up, *args, **kwargs)
    ...
    def _select_step(self, shots_0, shots_1, beta, beta_scale, current_objective_value, gradient, iteration, objective_arguments, **kwargs):
        ...
        for alpha_, m in zip(alphas, mem):
            rho, s, y = m
            beta_ = rho*y.inner_product(r) #Unfortunately a similar name as the regularization beta, so I added a trailing underscore '_' to alpha and beta
            r += (alpha_-beta_)*s
        ...
        alpha_ = self.select_alpha(shots_0, shots_1, beta, beta_scale, gradient, direction, objective_arguments,
    ...
    def _compute_alpha0(self, phi0, grad0, reset=False, *args, **kwargs):
        if reset:
            ...
            if self.scale_step and not self.did_grad_descent: 
                ...
                last_accepted_step_len = np.sqrt(np.linalg.norm(last_accepted_step.p_0.data)**2 + np.linalg.norm(last_accepted_step.p_1.data)**2)
                
                geom_fac_up = kwargs['upscale_factor']
                desired_new_step_len = last_accepted_step_len / geom_fac_up 
                
                current_new_step_len = np.sqrt(np.linalg.norm(self.unscaled_suggested_step.p_0.data)**2 + np.linalg.norm(self.unscaled_suggested_step.p_1.data)**2)
