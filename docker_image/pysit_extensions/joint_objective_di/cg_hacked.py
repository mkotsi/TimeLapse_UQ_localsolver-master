    ...
    def _select_step(self, shots_0, shots_1, beta, beta_scale, current_objective_value, gradient, iteration, objective_arguments, **kwargs):
        ...
        if (self.prev_gradient is None) or reset:
            ...
            if self.beta_style == 'fletcher-reeves':
                beta_ = gk.inner_product(gk) / gkm1.inner_product(gkm1) #The name beta unfortunately refers to my regularization already. so have to use beta_
            elif self.beta_style == 'polak-ribiere':
                beta_ = (gk-gkm1).inner_product(gk) / gkm1.inner_product(gkm1)

            direction = -1*gk + beta_*self.prev_direction
        ...
        alpha = self.select_alpha(shots_0, shots_1, beta, beta_scale, gradient, direction, objective_arguments,
