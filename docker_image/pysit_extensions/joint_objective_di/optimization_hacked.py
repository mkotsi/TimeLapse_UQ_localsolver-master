...
class OptimizationBase(object):
    ...
    def __init__(self, objective_0, objective_1, geom_fac = 0.6, geom_fac_up = 0.7):
        ...
        * InversionMethodsType must have member functions that implement the
        ...
        """
        self.objective_function_0 = objective_0
        self.objective_function_1 = objective_1
        
        assert(id(objective_0.solver) == id(objective_1.solver))
        assert(objective_0.use_parallel() == objective_1.use_parallel())
        
        self.solver = objective_0.solver
        self.verbose = False

        self.use_parallel = objective_0.use_parallel()

        self.max_linesearch_iterations = 20

        self.logfile = sys.stdout
        self.geom_fac = geom_fac
        self.geom_fac_up = geom_fac_up
    ...
    def store_history(self, arg, i, val, force=False):
        ...
        if self.use_parallel and (self.objective_function_0.parallel_wrap_shot.rank != 0):
            return
...
#   #### Actual optimization stuff below...
    ...
    def initialize(self, initial_value, **kwargs):
        ...
        if self.objective_function_0.parallel_wrap_shot.rank == 0:
            self._print("No longer setting the base model to the solver, because base model is now a joint model!")

    def __call__(self,
                 shots_0,
                 shots_1,
                 beta,
                 model_reg_term_scale,
                 initial_value,
        ...
        initial_value : A JointModel model object now!!!!
        ...
        if np.iterable(iteration_parameters):
            for ip in iteration_parameters:
                ...
                # Call the inner loop
                self.inner_loop(shots_0, shots_1, beta, model_reg_term_scale, steps, objective_arguments=arguments, **kwargs)
        else:
            if type(iteration_parameters) is int:
                # Call the inner loop
                steps=iteration_parameters
                self.inner_loop(shots_0, shots_1, beta, model_reg_term_scale, steps, **kwargs)
            else:
    ...
    def joint_objective(self, shots_0, shots_1, beta, model_reg_term_scale, model, objective_arguments={}):
        
        self.solver.model_parameters = model.m_0
        obj_m_0 = self.objective_function_0.evaluate(shots_0, model.m_0, **objective_arguments)
        
        self.solver.model_parameters = model.m_1
        obj_m_1 = self.objective_function_1.evaluate(shots_1, model.m_1, **objective_arguments)
        
        model_diff = model.m_0 - model.m_1 #perturbation
        take_norm_squared_of_this = model_diff*(1/beta) #ndarray
        take_norm_squared_of_this = model.m_0.perturbation(data=take_norm_squared_of_this) #make perturbation of it again
        #implement norm squared as inner product with itself! Inner product actually does the volume integral that I need to do! norm() does not do the volume integral, so not consistent !!! (in _backtrack_linesearch for instance I get the directional derivative <d\chi / dm , Delta m>_m by calling gradient.inner_product(tdir), which is a volume integral!  
        ...
        joint_objective = obj_m_0 + obj_m_1 + objective_value_contrib_from_beta

        return joint_objective

    def joint_gradient(self, shots_0, shots_1, beta, model_reg_term_scale, model, objective_arguments = {}, aux_info={}):
        #First for m_0
        
        if type(model) != JointModel:
            raise Exception("Wrong type for joint inversion!")
        
        aux_info_copy = copy.deepcopy(aux_info)
        
        self.solver.model_parameters = model.m_0
        gradient_m_0 = self.objective_function_0.compute_gradient(shots_0, model.m_0, aux_info=aux_info, **objective_arguments)
        objective_value_m_0 = aux_info['objective_value'][1]
        if 'pseudo_hess_diag' in aux_info.keys():
            h_0 = aux_info['pseudo_hess_diag'][1]

        #Now for m_1

        self.solver.model_parameters = model.m_1
        gradient_m_1 = self.objective_function_1.compute_gradient(shots_1, model.m_1, aux_info=aux_info_copy, **objective_arguments)
        objective_value_m_1 = aux_info_copy['objective_value'][1]
        if 'pseudo_hess_diag' in aux_info.keys():
            h_1 = aux_info['pseudo_hess_diag'][1]
        
        model_diff = model.m_0 - model.m_1 #subtraction gives a perturbation class back, so it is in linear form
        
        gradient_m_0_contrib_from_beta = model_diff*(model_reg_term_scale*(1/beta)**2)   #no 0.5* here. NDARRAY
        gradient_m_0_contrib_from_beta = model.m_0.perturbation(data=gradient_m_0_contrib_from_beta)
        
        gradient_m_1_contrib_from_beta = model_diff*(-model_reg_term_scale*(1/beta)**2)  #no 0.5* here. NDARRAY
        gradient_m_1_contrib_from_beta = model.m_1.perturbation(data=gradient_m_1_contrib_from_beta)
        ...
        joint_gradient = JointPerturbation(gradient_m_0 + gradient_m_0_contrib_from_beta, gradient_m_1 + gradient_m_1_contrib_from_beta)
        
        #objective value components was the squared norm. See temporal_least_squares.compute_gradient. So we can add
        joint_objective = objective_value_m_0 + objective_value_m_1 + objective_value_contrib_from_beta

        i = self.iteration
        self.store_history('gradient', i, joint_gradient)
        gradient_norm = joint_gradient.norm()
        ...
        if aux_info['objective_value'][1] is not None:
            self.store_history('objective', i, joint_objective)
            self._print('  objective {0}'.format(joint_objective))

        

        if aux_info['residual_norm'][1] is not None:
            residual_norm = np.sqrt(aux_info['residual_norm'][1]**2 + aux_info_copy['residual_norm'][1]**2)
            self.store_history('residual_length', i, residual_norm)
            self._print('  residual {0}'.format(residual_norm))
        
        if self.objective_function_0.parallel_wrap_shot.rank == 0:
            self._print("STORING HISTORY SHOULD PROBABLY NOT HAPPEN IN JOINT_GRADIENT FOR CONSISTENCY WITH JOINT_OBJECTIVE. SHOULD PUT BACK OUTSIDE IN INNER_LOOP. \n")
            
            #FOR DEBUGGING PURPOSES
            self._print('objective_value_m_0: {0}'.format(objective_value_m_0))
            self._print('objective_value_m_1: {0}'.format(objective_value_m_1))
            self._print('objective_value_contrib_from_beta: {0}'.format(objective_value_contrib_from_beta))
        
            self._print('gradient norm m_0: {0}'.format(gradient_m_0.norm()))
            self._print('gradient norm m_1: {0}'.format(gradient_m_1.norm()))
            self._print('gradient norm m_0 contrib from beta: {0}'.format(gradient_m_0_contrib_from_beta.norm()))
            self._print('gradient norm m_1 contrib from beta: {0}'.format(gradient_m_1_contrib_from_beta.norm()))
            

        if 'pseudo_hess_diag' in aux_info.keys():
            return joint_gradient, joint_objective, h_0, h_1
        else:
            return joint_gradient, joint_objective


    def inner_loop(self, shots_0, shots_1, beta, model_reg_term_scale, steps, objective_arguments={}, **kwargs):
        ...
        for step in xrange(steps):
            ...
            gradient, objective_value = self.joint_gradient(shots_0, shots_1, beta, model_reg_term_scale, self.base_model, objective_arguments, aux_info=aux_info)

            # Compute step modifier
            step = self._select_step(shots_0, shots_1, beta, model_reg_term_scale, objective_value, gradient, i, objective_arguments, **kwargs)
    ...
    def select_alpha(self, shots_0, shots_1, beta, model_reg_term_scale, gradient, direction, objective_arguments, **kwargs):
        """Resets the state of the optimization algorithm.
        ...
        if self.ls_method == 'constant':
            ...
            return self._linear_line_search(shots_0, shots_1, beta, model_reg_term_scale, gradient, direction, objective_arguments, **kwargs)

        elif self.ls_method == 'backtrack':
            return self._backtrack_line_search(shots_0, shots_1, beta, model_reg_term_scale, gradient, direction, objective_arguments, **kwargs)
    ...
    def _linear_line_search(self, shots_0, shots_1, beta, model_reg_term_scale, gradient, direction, objective_arguments, **kwargs):
...
#       return numerator / denominator
    ...
    def _backtrack_line_search(self, shots_0, shots_1, beta, model_reg_term_scale, gradient, direction, objective_arguments,
        ...
        if current_objective_value is None:
            fk = self.joint_objective(shots_0, shots_1, beta, model_reg_term_scale, self.base_model, **objective_arguments)
        else:
        ...
        while not stop:
            ...
            fkp1 = self.joint_objective(shots_0, shots_1, beta, model_reg_term_scale, model, **objective_arguments)
