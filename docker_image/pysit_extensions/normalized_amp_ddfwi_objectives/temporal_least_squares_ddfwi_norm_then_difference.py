...
__all__ = ['TemporalLeastSquaresDDFWINormUtoDThenDiff']

__docformat__ = "restructuredtext en"

class TemporalLeastSquaresDDFWINormUtoDThenDiff(TemporalLeastSquares):
    def __init__(self, solver, shots_d1, shots_d0, res_match_obj, parallel_wrap_shot=ParallelWrapShotNull(), imaging_period = 1, norm_both_to_d0 = False, normalize_d_to_u = False):
    ...
    def _residual(self, shot, m1, dWaveOp=None): #OVERRIDE. 
        ...
        if self.normalize_d_to_u:
            d0_normalized = match_amp_first_to_second(interpolated_d0, u0, self.solver.dt, self.res_match_obj)
            
            if self.norm_both_to_d0: #POORLY CHOSEN VARIABLE NAME NOW THAT I ALSO ALLOW D TO BE NORMALIZED TO U. THIS VARIABLE SHOULD MEAN NORMALIZE BOTH TO 0 
                d1_normalized = match_amp_first_to_second(interpolated_d1, u0, self.solver.dt, self.res_match_obj)
            else:
                d1_normalized = match_amp_first_to_second(interpolated_d1, u1, self.solver.dt, self.res_match_obj)
                
            delta_d = d1_normalized - d0_normalized
            delta_u = u1 - u0
        else:
