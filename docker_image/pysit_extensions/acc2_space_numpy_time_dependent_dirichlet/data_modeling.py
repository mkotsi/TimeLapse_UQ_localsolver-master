...
def generate_seismic_data(shots, solver, model, time_dirichlet_bc_array, node_order, verbose=False, frequencies=None, **kwargs):
    ...
    **kwargs : dict, optional
    ...
    for shot in shots:
        ...
        if solver.supports['equation_dynamics'] == "time":
            time_dir_bc_data = [time_dirichlet_bc_array[shots.index(shot),:,:],node_order]
            generate_shot_data_time(shot, solver, model, time_dir_bc_data,verbose=verbose, **kwargs)
        elif solver.supports['equation_dynamics'] == "frequency":
            raise Exception('This version deals with time data only')
...
def generate_shot_data_time(shot, solver, model, time_dir_bc_data, wavefields=None, wavefields_padded=None, verbose=False, **kwargs):
    ...
    solver_data = solver.SolverData(time_dir_bc_data)
